...
/*:
 * @target MV MZ
 * @plugindesc v1.022 Change the event's passage determination.
...
/*:ja
 * @target MV MZ
 * @plugindesc v1.022 イベントの通行判定を変更します。
...
Game_CharacterBase.prototype.isMapPassable = function(x, y, d) {
    ...
    if (setting) {
        ...
        // 未指定ならば、通常の判定を参照
        if (passable1 == null) {
            passable1 = $gameMap.isPassable(x, y, d);
        }
        if (passable2 == null) {
            passable2 = $gameMap.isPassable(x2, y2, this.reverseDir(d));
        }
